...
from utils import memory, logger, setup_genetic_algorithm, evaluate_individual_target
...
def optimize_target_ga(
    ...
    # Progress via st.progress in app; here, use rich or skip
    ...
    optimal_df[target_name] = predict_nn(model, scaler.transform(optimal_df))[0][0]
...
def optimize_target_bayesian(
    ...
    optimal_df = pd.DataFrame([optimal_params], columns=features.columns)
    optimal_df[target_name] = -res.fun
...
def optimize_target(
    ...
    config: Config, 
    optimizer: str = 'ga'
) -> Tuple[pd.DataFrame, List[plt.Figure | go.Figure], Tuple[np.ndarray, pd.DataFrame] | None]:
    ...
    from models import train_neural_network  # Local import to avoid cycle
    model, scaler = train_neural_network(features, target, config)
    ...
    figs = []
    # Plot NN importance (use visualization.py for full plot)
    # Placeholder: return importance_df for viz module
